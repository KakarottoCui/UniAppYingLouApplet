//微信：egvh56ufy7hh,QQ：821898835
		...
		<view class="pet-wrap u-p-20">
			...
			<view>
				<view class="item" v-for="(item, index) in items" :key="index" @tap="switchTab('/pages/product/list?id='+item.id)">
	...
	export default {
		data() {
			...
			getData(){
				...
				appRequest.request({
					method: "POST",
					url: appRequest.urlMap.findPhArticleList,
					data:{
						validFlag:1,
						type:0
					},
				...
				appRequest.request({
					method: "POST",
					url: appRequest.urlMap.addPhArticle,
					data:_this.form,
					success: function(res) {
						if (res.data.code == 200) {
							_this.getData();
